...
export class TripComponent {
  ...
  async buildTrip(from, to) {
    ...
    const uniqueRoutes2 = [...new Set(rows.map(row => row.route_no))];
    ...
    return {
      ...
      stops2: uniqueRoutes2
    };
  }


  async buildTrip2(from, to, route_no ) {
    const [rows] = await db.execute(
      'SELECT r.*, r.route_no, timediff(s2.arrial_time, s1.arrial_time) as arrial_time , st1.name as stop_name1, st2.name as stop_name2 FROM SCHEDULE s1 left join SCHEDULE s2 on s1.v_on_r_id=s2.v_on_r_id left join VEICHLES_ON_ROUTE v on s1.v_on_r_id=v.v_on_r_id left join ROUTE r on v.route_no=r.route_no left join STOP st1 on st1.stop_id=s1.stop_id left join STOP st2 on st2.stop_id=s2.stop_id  WHERE st1.name = ? AND st2.name = ? and r.route_no= ?',
            [from, to, route_no]
    );
